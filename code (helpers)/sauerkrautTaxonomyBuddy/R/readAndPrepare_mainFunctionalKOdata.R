...
#' Read and prepare all data of the functional KO stool measurements
#' 
#' This function wraps the functions for KO, blood marker and metabolome
...
#' 
#' @inheritParams readAndPrepare_mainFunctionalPathwayData
#' 
...
#' 
readAndPrepare_mainFunctionalKOdata <- function(path_KEGG_KO,
                                                path_KEGG_pathways,
                                                path_groupInfoRdata,
  ...
  datList_relFun <- readAndPrepare_functionalData(path_KEGG_KO             = path_KEGG_KO,
                                                  ...
                                                  exclude_sickParticipants = exclude_sickParticipants)
  dat_tpmFun <- datList_tpmFun[[paste0("datKO_",data_type,"Samples")]]
  dat_relFun <- datList_relFun[[paste0("datKO_",data_type,"Samples")]]
  ...
  dat_row           <- dat_tpmFun %>% 
    select(KO) %>% 
    mutate(label = "no labels assigned yet",
           label = factor(label)) %>% 
    as.data.frame()
  rownames(dat_row) <- dat_row$KO
  dat_row           <- dat_row %>% select(-KO)

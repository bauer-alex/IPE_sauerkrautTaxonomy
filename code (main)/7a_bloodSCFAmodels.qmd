...
dat_test <- dat_model %>% 
  ...
  summarize(group                  = first(group),
            interventionEffect_raw = blood_aceticAcid[2] - blood_aceticAcid[1]) %>% 
  ungroup() %>% 
  group_by(participant_id) %>% 
  summarize(group = first(group),
...
model_fibInt_list <- list("Acetic acid (Blood)"          = list(m_acetic_acid_fibInt, m_acetic_acid_fibInt_changedRef),
                          "Propionic acid (Blood)"       = list(m_propionic_acid_fibInt, m_propionic_acid_fibInt_changedRef),
                          "Butyric acid (Blood)"         = list(m_butyric_acid_fibInt, m_butyric_acid_fibInt_changedRef),
                          "Valeric acid (Blood)"         = list(m_valeric_acid_fibInt, m_valeric_acid_fibInt_changedRef),
                          "Hexanoic acid (Blood)"        = list(m_hexanoic_acid_fibInt, m_hexanoic_acid_fibInt_changedRef),
                          "Iso-butyric acid (Blood)"     = list(m_isobutyric_acid_fibInt, m_isobutyric_acid_fibInt_changedRef),
                          "Iso-valeric acid (Blood)"     = list(m_isovaleric_acid_fibInt, m_isovaleric_acid_fibInt_changedRef),
                          "2-Methylbutyric acid (Blood)" = list(m_methylbutyric_acid2_fibInt, m_methylbutyric_acid2_fibInt_changedRef))
model_divInt_list <- list("Acetic acid (Blood)"          = list(m_acetic_acid_divInt, m_acetic_acid_divInt_changedRef),
                          "Propionic acid (Blood)"       = list(m_propionic_acid_divInt, m_propionic_acid_divInt_changedRef),
                          "Butyric acid (Blood)"         = list(m_butyric_acid_divInt, m_butyric_acid_divInt_changedRef),
                          "Valeric acid (Blood)"         = list(m_valeric_acid_divInt, m_valeric_acid_divInt_changedRef),
                          "Hexanoic acid (Blood)"        = list(m_hexanoic_acid_divInt, m_hexanoic_acid_divInt_changedRef),
                          "Iso-butyric acid (Blood)"     = list(m_isobutyric_acid_divInt, m_isobutyric_acid_divInt_changedRef),
                          "Iso-valeric acid (Blood)"     = list(m_isovaleric_acid_divInt, m_isovaleric_acid_divInt_changedRef),
                          "2-Methylbutyric acid (Blood)" = list(m_methylbutyric_acid2_divInt, m_methylbutyric_acid2_divInt_changedRef))
...
gg_gaussian_fibInt <- plot_dat %>% 
  ...
  scale_x_continuous(breaks = 1:length(levels(plot_dat$model)), labels = levels(plot_dat$model)) +
  scale_y_continuous("standardized estimate", limits = ylim_gaussianInt,
                     breaks = c(-.9,-.45,0,.45,.9),
                     labels = c("-0.9", "-0.45", "0", "+0.45", "+0.9")) +
  scale_color_manual(values = col_vector, guide = "none") +
  scale_fill_manual(values = col_vector, guide = "none") +
...
# 2) interactions with baseline diversity
(gg_gaussian_divInt + gg_exp_divInt) + patchwork::plot_layout(nrow = 1, widths = c(.33,.67))
...
# 5) interactions with BMI
(gg_gaussian_bmiInt + gg_exp_bmiInt) + patchwork::plot_layout(nrow = 1, widths = c(.33,.67))
# ggsave("modelResults_bmiInteraction_bloodSCFAs.png", width = 14, height = 4)

# 6) interactions with fiber
(gg_gaussian_fibInt + gg_exp_fibInt) + patchwork::plot_layout(nrow = 1, widths = c(.33,.67))
# ggsave("modelResults_bmiInteraction_bloodSCFAs.png", width = 14, height = 4)
```
...
## Same plot, also including significant diversity interaction effects
...
# read the stool SCFA models results data
results_dat_stool        <- read.csv("7b_resultsDat_stoolSCFAmodels.csv")
results_dat_divInt_stool <- read.csv("7b_resultsDat_divInt_stoolSCFAmodels.csv")
...
# results_dat_fibInt_stool <- read.csv("7b_resultsDat_fibInt_stoolSCFAmodels.csv")
...
plot_dat_divInt_stool <- results_dat_divInt_stool %>%
  ...
  mutate(parameter  = case_when(parameter == "interventionFresh" ~ "fresh intervention",
                                ...
                                TRUE ~ parameter),
         param_type   = paste0("for div. ", div_group),
         param_type2 = case_when(div_group == min(div_group) ~ "interaction effect lower subgroup",
                                  TRUE                       ~ "interaction effect higher subgroup"),
         model      = gsub(model, pattern = " \\(Stool)", replacement = ""),
         model      = factor(model, levels = levels(plot_dat_gaussian$model)),
         xCoord_shift = case_when(div_group == min(div_group) ~ -0.2,
                                  TRUE                        ~ 0.2),
         CI_alpha     = 0.3)

# prepare the interaction datasets for the blood models
plot_dat_divInt_blood <- results_dat_divInt %>%
  group_by(model, parameter) %>% 
  ...
  mutate(parameter  = case_when(parameter == "interventionFresh"       ~ "fresh intervention",
         ...
         model      = factor(model, levels = levels(plot_dat_gaussian$model)),
         xCoord_shift = case_when(div_group == min(div_group) & (model == "Acetic acid") ~ -0.2,
                                  model == "Acetic acid"                                 ~ 0.2,
                                  div_group == min(div_group) ~ -0.4,
                                  TRUE                        ~ 0.4),
         CI_alpha     = 0.3)
```
...
plot_dat_stoolGaussian <- results_dat_stool %>% 
  ...
  dplyr::bind_rows(plot_dat_divInt_stool %>% filter(model_type == "Gaussian")) %>% 
...
plot_dat_stoolExp <- results_dat_stool %>% 
  ...
  mutate(parameter = factor(parameter, levels = c("interventionFresh", "interventionPasteurized"),
         ...
         param_type2 = "main effect") %>% 
  dplyr::bind_rows(plot_dat_divInt_stool %>% filter(model_type != "Gaussian")) %>% 
  mutate(param_type = factor(param_type, levels = c("overall", "for div. [2.8, 3.85)", "for div. [3.85, 4.4)")),
...
plot_dat_bloodGaussian <- results_dat %>% 
  ...
  mutate(parameter  = factor(parameter, levels = c("interventionFresh", "interventionPasteurized"),
         ...
         model      = factor(model, levels = levels(plot_dat_gaussian$model)),
         model_xCoord = models_xCoords[["Gaussian"]][as.character(model)]) %>% 
  dplyr::bind_rows(plot_dat_divInt_blood %>% filter(model_type == "Gaussian")) %>% 
  mutate(param_type2 = factor(param_type2, levels = c("main effect", "interaction effect lower subgroup", "interaction effect higher subgroup")))
...
### plots
xlim_exp <- c(plot_dat_bloodExp$model_xCoord, plot_dat_stoolExp$model_xCoord) %>% range()
...
gg_bloodExp <- plot_dat_bloodExp %>% 
  ...
  scale_y_continuous("estimate, on log2 scale", trans = "log2", limits = c(.6,1.4),
                     ...
                     labels = c("-20%", "0", "+20%"),
                     minor_breaks = c(.6, 1.4)) +
  scale_color_manual("effect type", values = c("gray35", "#52A382", "#52A382")) + # "#A09E54", "#A09E54", "#B876BC", "#B876BC")) +
  scale_fill_manual("effect type", values = c("gray35", "#52A382", "#52A382")) + # "#A09E54", "#A09E54", "#B876BC", "#B876BC")) +
...
# ggsave("Figure5_bloodAndStoolSCFAmodels.png", width = 12, height = 6)
```
...
# ggsave("FigureS13_bloodSCFAs_stratifications.png", width = 16, height = 16)
```

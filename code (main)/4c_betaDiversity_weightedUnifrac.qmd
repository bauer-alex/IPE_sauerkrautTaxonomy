---
title: "Beta diversity -- weighted UniFrac distance"
format:
  html:
    toc: true
    code-fold: true
...
The pairwise weighted UniFrac distances in our sample vary between
`r round(min(datDist_intra$uniFrac_weighted), 2)` and `r round(max(datDist_intra$uniFrac_weighted), 2)`
regarding the intra-personal variability and between
`r round(min(datDist_inter$uniFrac_weighted), 2)` and `r round(max(datDist_inter$uniFrac_weighted), 2)`
regarding the inter-personal variability.
The weighted UniFrac distances have the following distribution:

```{r}
#| message: false
#| warning: false

gg1 <- ggplot(datDist_intra, aes(x = uniFrac_weighted)) + geom_histogram() + xlim(c(0,1)) + ggtitle("Intra-personal weighted UniFrac distances")
gg2 <- ggplot(datDist_inter, aes(x = uniFrac_weighted)) + geom_histogram() + xlim(c(0,1)) + ggtitle("Inter-personal weighted UniFrac distances")
...
1. For each timepoint comparison (e.g. *T1 vs. T3*) one weighted UniFrac distance was
...
datDist_intra %>% 
  ...
  ggplot(aes(x = comparison, y = uniFrac_weighted)) +
  geom_boxplot() +
...
datDist_inter %>% 
  ...
  ggplot(aes(x = timepoint, y = uniFrac_weighted)) +
  geom_boxplot() +
  ylim(c(0,1)) +
  theme(axis.title.x = element_blank())
```
...
datDist_intra %>% 
  ...
  geom_boxplot() +
  scale_y_continuous("Weighted Unifrac dissimilarity") +

---
title: "Description Microbiome Changes with focus on genus level"
format:
  html:
    toc: true
    code-fold: true
...
# Changes on Sauerkraut genus level -- overall

In the following there's one plot and table for each of the 20 most common bacteria genus'. 
The genus' are sorted from most common to least common (based on all our samples).
...
# summarize per genus
dat_relAbd <- dat_relAbd %>% 
  group_by(participant_id, inter_treat, genus) %>% 
...
# create a vector of the most common bacteria genus'
genus_vec <- dat_relAbd %>% 
  group_by(genus) %>% 
  summarize(relAbd = sum(relative_abundance, na.rm = TRUE)) %>% 
  arrange(desc(relAbd)) %>% 
  slice(1:20) %>% 
  pull(genus) %>% 
  as.character()

# restrict the data to these genus' and remove the follow-up measurements
dat_relAbd_genus <- dat_relAbd %>% 
  filter(genus %in% genus_vec) %>% 
...
results_list <- lapply(genus_vec, function(x) {
  if (!(x %in% dat_relAbd_genus$genus))
    return(paste("genus", x, "is not appearing in the data."))
  
  # plot and development table
  generalResults_list <- dat_relAbd_genus %>% 
    filter(genus == x) %>% 
  ...
  # add the information how many samples had a relative abundance >= 1%
  tab_1abundance <- dat_relAbd_genus %>% 
    filter(genus == x) %>% 
    group_by(intervention, treatment) %>% 
    summarize(share_atLeast1percent = sum(relative_abundance >= 1) / n()) %>% 
    mutate(share_atLeast1percent = paste0(round(100 * share_atLeast1percent, 1), "%"))
  
  list(ggplot_object  = generalResults_list$ggplot_object,
       results_table  = generalResults_list$results_table,
       tab_1abundance = tab_1abundance)
})
```
...
results_list[[1]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[2]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[3]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[4]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[5]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[6]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[7]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[8]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[9]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[10]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[11]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[12]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[13]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[14]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[15]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[16]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[17]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[18]]$tab_1abundance %>% kable() %>% kable_styling()
...
results_list[[19]]$tab_1abundance %>% kable() %>% kable_styling()

results_list[[20]]$ggplot_object
results_list[[20]]$results_table %>% select(intervention, contains("share"))                       %>% kable() %>% kable_styling()
results_list[[20]]$results_table %>% select(intervention, contains("median"), -contains("Factor")) %>% kable() %>% kable_styling()
results_list[[20]]$results_table %>% select(intervention, contains("Factor"))                      %>% kable() %>% kable_styling()
results_list[[20]]$tab_1abundance %>% kable() %>% kable_styling()
```
